...
// 子表 排序相关
    var CONFIG = {
        ...
        //排序跟数据库实际字段名的映射
        sortMap:{
            adminUserId:'admin_user_id',userName:'user_name',tableName:'table_name',operateType:'operate_type',url:'url',createTime:'create_time'
        }
    };
    //后端请求时候的表模块规则url
    ...
    var COLS = [[
        {type: "checkbox", /*fixed:"left",*/ width:50},//全选
                {field: 'adminUserId', title: '管理员', align:'center', minWidth:250, templet:function (d) {
                    var value = (!d.adminUserId || d.adminUserId==null) ? '' : d.adminUserId
                    //var option = '<option selected="selected" value="'+value+'">'+value+'</option>'
                    return '<select class="magicalcoder-table-foreign-select2 layui-input security_list_table_form_adminUserId" lay-ignore="true"  name="adminUserId" data-identify="'+d.id+'" data-value="'+value+'" data-url="admin/sys_admin_user_rest/search" data-id="id" data-text-fields="username" lay-verify="magicalCoderVerify" magicalcoder-verify=""  placeholder="请输入管理员"  data-limit="20"></select>'
                    },sort:true
                },

            {field: 'userName', title: '管理员名称', minWidth:200,templet:function (d) {
                    return '<input type="text" value="'+ mc_util.escapeHTML(d.userName) +'" class="magicalcoder-table-text layui-input security_list_table_form_userName" name="userName" data-identify="'+d.id+'" lay-verify="magicalCoderVerify" magicalcoder-verify=""  placeholder="请输入管理员名称"/>'
                }
                , sort:true
            },

            {field: 'tableName', title: '表名', minWidth:200,templet:function (d) {
                    return '<input type="text" value="'+ mc_util.escapeHTML(d.tableName) +'" class="magicalcoder-table-text layui-input security_list_table_form_tableName" name="tableName" data-identify="'+d.id+'" lay-verify="magicalCoderVerify" magicalcoder-verify=""  placeholder="请输入表名"/>'
                }
                , sort:true
            },

            {field: 'operateType', title: '操作类型', minWidth:200,templet:function (d) {
                    return '<input type="text" value="'+ mc_util.escapeHTML(d.operateType) +'" class="magicalcoder-table-text layui-input security_list_table_form_operateType" name="operateType" data-identify="'+d.id+'" lay-verify="magicalCoderVerify" magicalcoder-verify="|minLength=0"  placeholder="请输入操作类型"/>'
                }
                , sort:true
            },

            {field: 'url', title: '链接', minWidth:200,templet:function (d) {
                    return '<input type="text" value="'+ mc_util.escapeHTML(d.url) +'" class="magicalcoder-table-text layui-input security_list_table_form_url" name="url" data-identify="'+d.id+'" lay-verify="magicalCoderVerify" magicalcoder-verify=""  placeholder="请输入链接"/>'
                }
                , sort:true
            },
