...
// 子表 排序相关
    var CONFIG = {
        ...
        childrenPage:[//子页面的一些基础信息 更多信息
            {
                tabTitle:"左侧二级菜单",
                url:"admin/page_v2/sys_module/list",
    ...
    var COLS = [[
        {type: "checkbox", /*fixed:"left",*/ width:50},//全选
            ...
            {field: 'moduleCategoryName', title: '一级菜单名称', minWidth:200,templet:function (d) {
                    return '<input type="text" value="'+ mc_util.escapeHTML(d.moduleCategoryName) +'" class="magicalcoder-table-text layui-input security_list_table_form_moduleCategoryName" name="moduleCategoryName" data-identify="'+d.id+'" lay-verify="magicalCoderVerify" magicalcoder-verify=""  placeholder="请输入一级菜单名称"/>'
                }
                , sort:true
            },

            {field: 'sortNum', title: '排序', minWidth:200,templet:function (d) {
                    return '<input type="text" value="'+ mc_util.escapeHTML(d.sortNum) +'" class="magicalcoder-table-text layui-input security_list_table_form_sortNum" name="sortNum" data-identify="'+d.id+'" lay-verify="magicalCoderVerify|mc_number" magicalcoder-verify=""  placeholder="请输入排序"/>'
                }
                , sort:true
            },
