...
// 子表 排序相关
    var CONFIG = {
        ...
        childrenPage:[//子页面的一些基础信息 更多信息
            {
                tabTitle:"系统日志",
                url:"admin/page_v2/sys_log_admin_operate/list",
        ...
        //排序跟数据库实际字段名的映射
        sortMap:{
            username:'username',createTime:'create_time',updateTime:'update_time',roleId:'role_id',enabled:'enabled'
        }
    };
    //后端请求时候的表模块规则url
    ...
    var COLS = [[
        {type: "checkbox", /*fixed:"left",*/ width:50},//全选
            ...
            {field: 'username', title: '用户名', minWidth:200,templet:function (d) {
                    return '<input type="text" value="'+ mc_util.escapeHTML(d.username) +'" class="magicalcoder-table-text layui-input security_list_table_form_username" name="username" data-identify="'+d.id+'" lay-verify="magicalCoderVerify|mc_required" magicalcoder-verify="|minLength=0"  placeholder="请输入用户名"/>'
                }
                , sort:true
            },
            ...
            {field: 'updateTime', title: '更新时间', align:'center', minWidth:250, templet :"#updateTimeTemplate",sort:true},
                {field: 'roleId', title: '角色', align:'center', minWidth:250, templet:function (d) {
                    ...
                    return '<select class="magicalcoder-table-foreign-select2 layui-input security_list_table_form_roleId" lay-ignore="true"  name="roleId" data-identify="'+d.id+'" data-value="'+value+'" data-url="admin/sys_role_rest/search" data-id="id" data-text-fields="roleName" lay-verify="magicalCoderVerify" magicalcoder-verify="|minLength=0"  placeholder="请输入角色"  data-limit="20"></select>'
                    },sort:true
                },

                {field: 'enabled', title: '是否可用',minWidth:150, align:'center', templet:function(d){
                    var checked = (d.enabled+''=='true' || d.enabled+'' == '1') ? 'checked' : '';
                    return '<input type="checkbox" class="magicalcoder-table-layswitch security_list_table_form_enabled" data-identify="'+d.id+'" name="enabled"  lay-filter="enabledFilter" lay-skin="switch" lay-text=""  '+checked+' lay-verify="magicalCoderVerify|mc_required" magicalcoder-verify="|minLength=0" />'
                    }
                    ,sort:true
                },
