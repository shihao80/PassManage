...
// 子表 排序相关
    var CONFIG = {
        ...
        //排序跟数据库实际字段名的映射
        sortMap:{
            id:'id',permitName:'permit_name',backendUrlReg:'backend_url_reg',moduleId:'module_id'
        }
    };
    //后端请求时候的表模块规则url
    ...
    var COLS = [[
        {type: "checkbox", /*fixed:"left",*/ width:50},//全选
            ...
            {field: 'permitName', title: '通过名称', minWidth:200,templet:function (d) {
                    return '<input type="text" value="'+ mc_util.escapeHTML(d.permitName) +'" class="magicalcoder-table-text layui-input security_list_table_form_permitName" name="permitName" data-identify="'+d.id+'" lay-verify="magicalCoderVerify|mc_required" magicalcoder-verify="|minLength=0"  placeholder="请输入通过名称"/>'
                }
                , sort:true
            },

            {field: 'backendUrlReg', title: '后端的地址正则', minWidth:200,templet:function (d) {
                    return '<input type="text" value="'+ mc_util.escapeHTML(d.backendUrlReg) +'" class="magicalcoder-table-text layui-input security_list_table_form_backendUrlReg" name="backendUrlReg" data-identify="'+d.id+'" lay-verify="magicalCoderVerify|mc_required" magicalcoder-verify="|minLength=0"  placeholder="请输入后端的地址正则,校验当前请求url是否有权限,get|post统一会按照参数首字母排序"/>'
                }
                , sort:true
            },
                {field: 'moduleId', title: '所属模块', align:'center', minWidth:250, templet:function (d) {
                    ...
                    return '<select class="magicalcoder-table-foreign-select2 layui-input security_list_table_form_moduleId" lay-ignore="true"  name="moduleId" data-identify="'+d.id+'" data-value="'+value+'" data-url="admin/sys_module_rest/search" data-id="id" data-text-fields="moduleTitle" lay-verify="magicalCoderVerify" magicalcoder-verify="|minLength=0"  placeholder="请输入所属模块"  data-limit="20"></select>'
                    },sort:true
                },
