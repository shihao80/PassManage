...
import com.passManage.us.admin.rmp.dto.*;
import com.passManage.us.admin.rmp.model.*;
import com.passManage.us.admin.rmp.service.*;
...
public class SecurityUserDetailServiceImpl implements SecurityUserDetailService {
    ...
    private List<SysModuleCategoryDto> getModuleCategoryDtoList(List<SysModuleDto> moduleDtoList){
        ...
        if(ListUtil.isNotBlank(moduleDtoList)){
            ...
            for(SysModuleCategory moduleCategory:categoryList){
                if(moduleCategory.getSortNum()==null){
                    moduleCategory.setSortNum(0);
                }
            }
    ...
    private List<SysModuleDto> getModuleDtoList(Set<Long> moduleIds,List<SysRoleModulePermission> sysRoleModulePermissionList){
        ...
        if(moduleList!=null){
            ...
            List<SysPermissionDto> permissionDtoList = getAllPermissionDtoList(moduleIds);
            Map<Long,List<SysPermissionDto>> modulePermissionListMap = MapUtil.listToListMap("moduleId",permissionDtoList);
            ...
            for(SysModule sysModule:moduleList){
                ...
                //当前模块有哪些权限
                List<SysPermissionDto> allPermissionDtoList = modulePermissionListMap.get(moduleId);
    ...
    private List<SysPermissionDto> getAllPermissionDtoList(Set<Long> moduleIds){
        if(moduleIds.isEmpty()){
            return  new ArrayList<>();
        }
        List<SysPermission> permissionList = sysPermissionService.getModelList(MapUtil.buildMap("moduleIds",moduleIds));
