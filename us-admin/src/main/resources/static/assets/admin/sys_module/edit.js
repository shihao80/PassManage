    ...
    var obj = {
                ...
                $.getJSON('admin/'+tableNameRest+"/get/"+identify,{date:new Date().getTime()},function (data) {//获取此实体详情数据
                    if(data.flag){
                        ...
                        if(entity && entity!=null){
                            ...
                            //禁用外键编辑区域：只有当从更多信息点击进来 才会禁用 单表操作不会执行 这里不要禁用
        ...
        submit : function () {
            ...
            form.on("submit(save)",function(data){
                ...
                $.post('admin/'+tableNameRest+'/extra_save',postData,function(res){
                    top.layer.close(index);
                    if(res.flag){
                        if(postData.helpConfigPermit){
                            top.layer.msg("成功！请至【角色管理】-【分配权限】-关联此模块权限");
                        }
