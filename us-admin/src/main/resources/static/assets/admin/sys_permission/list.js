...
// 子表 排序相关
    var CONFIG = {
        ...
        childrenPage:[//子页面的一些基础信息 更多信息
            {
                tabTitle:"角色模块权限",
                url:"admin/page_v2/sys_role_module_permission/list",
        ...
        //排序跟数据库实际字段名的映射
        sortMap:{
            filterPlatform:'filter_platform',permissionName:'permission_name',backendUrlReg:'backend_url_reg',frontDom:'front_dom',frontAction:'front_action',moduleId:'module_id'
        }
    };
    //后端请求时候的表模块规则url
    ...
    var COLS = [[
        {type: "checkbox", /*fixed:"left",*/ width:50},//全选
                {field: 'moduleId', title: '所属二级菜单', align:'center', minWidth:250, templet:function (d) {
                    var value = (!d.moduleId || d.moduleId==null) ? '' : d.moduleId
                    //var option = '<option selected="selected" value="'+value+'">'+value+'</option>'
                    return '<select class="magicalcoder-table-foreign-select2 layui-input security_list_table_form_moduleId" lay-ignore="true"  name="moduleId" data-identify="'+d.id+'" data-value="'+value+'" data-url="admin/sys_module_rest/search" data-id="id" data-text-fields="moduleTitle" lay-verify="magicalCoderVerify" magicalcoder-verify="|minLength=0"  placeholder="请选择所属二级菜单"  data-limit="20"></select>'
                    },sort:true
                },

            {field: 'permissionName', title: '权限名称', minWidth:200,templet:function (d) {
                    return '<input type="text" value="'+ mc_util.escapeHTML(d.permissionName) +'" class="magicalcoder-table-text layui-input security_list_table_form_permissionName" name="permissionName" data-identify="'+d.id+'" lay-verify="magicalCoderVerify" magicalcoder-verify="|minLength=0"  placeholder="请输入权限名称"/>'
                }
                , sort:true
            },
            {field: 'frontAction', title: '前端处理方式',minWidth:250, align:'center', templet:'#frontActionTemplate',sort:true},
            {field: 'filterPlatform', title: '规则适用端',minWidth:250, align:'center', templet:'#filterPlatformTemplate',sort:true},

            {field: 'frontDom', title: 'DOM前端Key', minWidth:300,templet:function (d) {
                    return '<input type="text" value="'+ mc_util.escapeHTML(d.frontDom) +'" class="magicalcoder-table-text layui-input security_list_table_form_frontDom" name="frontDom" data-identify="'+d.id+'" lay-verify="magicalCoderVerify" magicalcoder-verify=""  placeholder="jquery选择器，逗号分隔多个，例如#id,[name=\'goodsName\']"/>'
                }
                , sort:true
            },

            {field: 'backendUrlReg', title: '后端正则地址', minWidth:300,templet:function (d) {
                    return '<input type="text" value="'+ mc_util.escapeHTML(d.backendUrlReg) +'" class="magicalcoder-table-text layui-input security_list_table_form_backendUrlReg" name="backendUrlReg" data-identify="'+d.id+'" lay-verify="magicalCoderVerify" magicalcoder-verify=""  placeholder="/admin/表名_rest/.* 请输入后端的地址正则,校验当前请求url是否有权限,get|post统一会按照参数首字母排序"/>'
                }
                , sort:true
            },
        {title: '操作', minWidth:200, templet:'#newsListOperateTemplate',/*fixed:"right",*/align:"center"}//操作 到list.html页面查找模板对应的html
    ]];
    var obj = {
